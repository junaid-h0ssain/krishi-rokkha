// JS companion for tests and simple imports (falls back to mock if config not present)
// Do not import the Vite `config.js` at top-level because it uses `import.meta.env`
// which is undefined in plain Node. Instead read from `globalThis.WEATHER_CONFIG`
// or environment variables when running tests.
...
export async function fetchWeatherForDistrict(district) {
  ...
  const cfg = (typeof globalThis !== 'undefined' && (globalThis.WEATHER_CONFIG)) || {
    baseUrl: process.env.VITE_WEATHER_BASE_URL,
    apiKey: process.env.VITE_WEATHER_API_KEY
  };

  if (!cfg || !cfg.baseUrl) return getMockWeatherData();

  const url = `${cfg.baseUrl}?lat=${coords.lat}&lon=${coords.lon}&appid=${cfg.apiKey}&units=metric`;
  try {
...
function aggregateToDaily(list) {
  ...
  return Object.keys(byDay).slice(0, 5).map((dateKey) => {
    ...
    return {
      ...
      temp: avg(items.map((i) => (i.main?.temp ?? i.temp ?? 0))),
      humidity: avg(items.map((i) => (i.main?.humidity ?? 0))),
...
export function makeBanglaAdvice(day) {
